...
Ejecutar con: streamlit run 01_eda_streamlit.py
"""
...
def load_data(path: str) -> pd.DataFrame:
    ...
    # Variables derivadas (obligatorias para el examen)
    df["age_group"] = pd.cut(
        df["age"],
...
with st.sidebar:
    ...
    if data_source == "Subir archivo":
        ...
        # Busca el archivo en el mismo directorio que este script
...
def rate_by(df, col):
    """Tasa de >50K por categoría, ordenada descendente."""
    return (
...
if seccion == "1 · Visión General":
    ...
    # Tasa por cuartil de variables numéricas
    with col2:
        ...
        quartile_rates = {}
        for v in num_vars:
            qcut = pd.qcut(df[v], q=4, labels=["Q1", "Q2", "Q3", "Q4"])
            quartile_rates[v] = (
...
# ─────────────────────────────────────────────
    ...
    with tab3:
        ...
        # Reindexar según EDU_ORDER que estén presentes
...
# ─────────────────────────────────────────────
    ...
    with tab2:
        ...
        # Curva de probabilidad por edad (smooth)
